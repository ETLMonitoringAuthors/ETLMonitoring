...
def compute_embedding_distances_external(frames, encoder, target_frame, distance_metric='cosine'):
	"""
	Compute distances between external target frame embedding and all frame embeddings.

	Args:
		frames: Tensor of shape [T, C, H, W] containing episode frames
		encoder: PretrainedEncoder instance
		target_frame: External target frame of shape [C, H, W] (loaded from disk)
	...
	Returns:
		...
		pixel_mse: Tensor of shape [T] containing pixel MSE to target frame
	"""
	# Encode all frames from trajectory
	# frames should be [T, C, H, W] in uint8
	embeddings = encoder(frames)  # [T, 768]

	# Encode external target frame
	# Add batch dimension [1, C, H, W] for encoder
	target_frame_batched = target_frame.unsqueeze(0)  # [1, C, H, W]
	target_embedding = encoder(target_frame_batched)  # [1, 768]
	...
	# Compute pixel MSE between each frame and external target frame
	target_frame_float = target_frame.unsqueeze(0).float()  # [1, C, H, W]
	frames_float = frames.float()  # [T, C, H, W]
	# MSE over all pixels (C, H, W dimensions)
	pixel_mse = ((frames_float - target_frame_float) ** 2).mean(dim=(1, 2, 3))  # [T]

	return distances.cpu(), embeddings.cpu(), pixel_mse.cpu()


def plot_embedding_distances(distances_list, task_names, pixel_mse_list, save_path, distance_metric='cosine', external_target=True):
	"""
	...
	Args:
		...
		external_target: Whether the target is an external frame (vs. trajectory frame)
	"""
	...
	for idx, (distances, pixel_mse, task_name) in enumerate(zip(distances_list, pixel_mse_list, task_names)):
		...
		# Mark the minimum distance frame (closest to target)
		min_dist_idx = distances.argmin().item()
		ax1.axvline(x=min_dist_idx, color='green', linestyle='--', linewidth=2,
				   label=f'Closest to Target (t={min_dist_idx})', alpha=0.7)
		ax1.scatter([min_dist_idx], [distances[min_dist_idx]], color='green', s=150, zorder=5, marker='o')
		...
		target_label = 'External Target Frame' if external_target else 'Goal Frame'
		ax1.set_title(f'Task: {task_name} | Embedding Distance to {target_label}', fontsize=14, fontweight='bold')
		...
		# Mark the minimum MSE frame
		min_mse_idx = pixel_mse.argmin().item()
		ax2.axvline(x=min_mse_idx, color='green', linestyle='--', linewidth=2,
				   label=f'Closest to Target (t={min_mse_idx})', alpha=0.7)
		ax2.scatter([min_mse_idx], [pixel_mse[min_mse_idx]], color='green', s=150, zorder=5, marker='o')
		...
		ax2.set_title(f'Task: {task_name} | Pixel MSE to {target_label}', fontsize=14, fontweight='bold')
		ax2.set_ylim(bottom=0)
		ax2.grid(True, alpha=0.3)
		ax2.legend(fontsize=10, loc='best')

		# Add statistics for pixel MSE
		mean_mse = pixel_mse.mean().item()
		std_mse = pixel_mse.std().item()
		min_mse = pixel_mse.min().item()
		max_mse = pixel_mse.max().item()

		if len(pixel_mse) > 1:
			start_mse = pixel_mse[0].item()
			end_mse = pixel_mse[-1].item()
...
def plot_embedding_trajectories_2d(embeddings_list, task_names, target_embedding, save_path):
	"""
	...
	Args:
		...
		target_embedding: External target embedding of shape [768]
		save_path: Path to save the plot
	"""
	...
	# Concatenate all embeddings + target embedding for PCA
	all_embeddings = torch.cat(embeddings_list + [target_embedding.unsqueeze(0)], dim=0).numpy()

	# Apply PCA to reduce to 2D
	pca = PCA(n_components=2)
	all_embeddings_2d = pca.fit_transform(all_embeddings)

	# Split back into individual episodes + target
	embeddings_2d_list = []
	start_idx = 0
	for embeddings in embeddings_list:
		...
		start_idx = end_idx
	target_embedding_2d = all_embeddings_2d[-1]  # Last one is target
	...
	for idx, (embeddings_2d, task_name) in enumerate(zip(embeddings_2d_list, task_names)):
		...
		# Highlight external target
		ax.scatter(target_embedding_2d[0], target_embedding_2d[1],
				  color='red', s=300, marker='*', label='External Target', zorder=5, edgecolors='black', linewidths=2)
...
def plot_embedding_trajectories_3d_umap(embeddings_list, task_names, target_embedding, save_path):
	"""
	...
	# Concatenate all embeddings + target for UMAP
	all_embeddings = torch.cat(embeddings_list + [target_embedding.unsqueeze(0)], dim=0).numpy()
	...
	for embeddings in embeddings_list:
		...
		start_idx = end_idx
	target_embedding_3d = all_embeddings_3d[-1]  # Last one is target
	...
	for idx, (embeddings_3d, task_name) in enumerate(zip(embeddings_3d_list, task_names)):
		...
		# Highlight external target (red star)
		ax.scatter(target_embedding_3d[0], target_embedding_3d[1], target_embedding_3d[2],
				  color='red', s=400, marker='*', label='External Target', zorder=10,
				  edgecolors='black', linewidths=2)
...
def generate_demos(cfg):
	"""Generates demonstrations with embedding distance analysis using an external target frame."""
	...
	# Check for target frame path
	assert hasattr(cfg, 'target_frame_path'), \
		'Please specify path to target frame via +target_frame_path=<path_to_image>'
	target_frame_path = cfg.target_frame_path
	...
	# Load external target frame
	print(f'Loading external target frame from {target_frame_path}...')
	target_frame = load_target_frame(target_frame_path)  # [C, H, W]

	# Encode target frame once
	target_frame_cuda = target_frame.unsqueeze(0).to('cuda')  # [1, C, H, W]
	target_embedding = vision_encoder(target_frame_cuda).squeeze(0).cpu()  # [768]
	...
	# Generate demos
	print(f'Generating {cfg.num_demos} demonstrations with external target embedding analysis...')
	demos_collected = 0
		...
		if done.any():
			...
			for i in range(cfg.num_envs):
				...
				if demos_collected >= cfg.num_demos:
					...
					# Compute embedding distances to external target frame
					print(f'Computing embedding distances to external target for demo {demos_collected+1}...')
					distances, embeddings, pixel_mse = compute_embedding_distances_external(
						ep_frames.to('cuda'), vision_encoder, target_frame.to('cuda'), distance_metric=distance_metric)
					distances_list.append(distances)
					embeddings_list.append(embeddings)
					pixel_mse_list.append(pixel_mse)
					accepted_task_names.append(cfg.tasks[i])

					# Print closest frame to target
					min_dist_idx = distances.argmin().item()
					print(f'Closest frame to target: t={min_dist_idx} (distance={distances[min_dist_idx]:.4f})')
	...
	plot_save_path = f'{cfg.data_dir}/{cfg.task}_embedding_distances_external_{distance_metric}.png'
	plot_embedding_distances(distances_list, accepted_task_names, pixel_mse_list, plot_save_path, distance_metric=distance_metric, external_target=True)

	# Plot embedding trajectories (PCA 2D)
	print('Generating embedding trajectory plots (PCA 2D)...')
	trajectory_save_path = f'{cfg.data_dir}/{cfg.task}_embedding_trajectories_external_{distance_metric}.png'
	plot_embedding_trajectories_2d(embeddings_list, accepted_task_names, target_embedding, trajectory_save_path)

	# Plot embedding trajectories (UMAP 3D)
	print('Generating embedding trajectory plots (UMAP 3D)...')
	trajectory_save_path_umap = f'{cfg.data_dir}/{cfg.task}_embedding_trajectories_umap3d_external_{distance_metric}.png'
	plot_embedding_trajectories_3d_umap(embeddings_list, accepted_task_names, target_embedding, trajectory_save_path_umap)
	...
	print(f'Target frame used: {target_frame_path}')
