...
def generate_demos(cfg):
	"""Generates demonstrations with embedding distance analysis and saves individual frames."""
		...
		if done.any():
			...
			for i in range(cfg.num_envs):
				...
				if demos_collected >= cfg.num_demos:
					...
					# FIX: Append the squeezed frames instead of ep_td['frame']
	...
	# Stack frames list into a tensor
	frames = torch.stack(frames, dim=0)  # Now shape: [num_demos, T+1, C, H, W]
	...
	plot_embedding_distances(distances_list, accepted_task_names, done_indices_list, pixel_mse_list, plot_save_path, distance_metric=distance_metric)
	...
	for demo_idx in range(frames.shape[0]):
		...
		# Create subdirectory for this demo
		demo_dir = f'{cfg.data_dir}/{cfg.task}_demo_{demo_idx:03d}'
		...
		for frame_idx in range(demo_frames.shape[0]):
			...
			save_image(frame, f'{demo_dir}/frame_{frame_idx:03d}.png')

		print(f'Saved demo {demo_idx+1}/{demos_collected} with {demo_frames.shape[0]} frames to {demo_dir}/')

	print(f'\nCompleted! Saved {demos_collected} demos with individual frames to {cfg.data_dir}.')
