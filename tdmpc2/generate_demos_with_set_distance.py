...
def load_target_frames(target_dir):
	"""
	Load all target frames from a directory.

	Args:
		target_dir: Path to directory containing target frame images

	Returns:
		target_frames: Tensor of shape [N, C, H, W] in uint8 (0-255 range)
	"""
	assert os.path.isdir(target_dir), f"Target frames directory not found at {target_dir}"

	# Find all image files
	extensions = ['*.png', '*.jpg', '*.jpeg', '*.PNG', '*.JPG', '*.JPEG']
	image_paths = []
	for ext in extensions:
		image_paths.extend(sorted(glob_module.glob(os.path.join(target_dir, ext))))

	assert len(image_paths) > 0, f"No image files found in {target_dir}"
	...
	for path in image_paths:
		...
		frames.append(frame)
		print(f"  Loaded: {os.path.basename(path)} (shape: {frame.shape})")

	target_frames = torch.stack(frames, dim=0)  # [N, C, H, W]
	print(f"Loaded {len(frames)} target frames from {target_dir}")
	print(f"Target frames tensor shape: {target_frames.shape}")

	return target_frames


def compute_set_embedding_distances(frames, encoder, target_frames, distance_metric='cosine'):
	"""
	Compute set-based distances between trajectory frames and a set of goal frames.

	Args:
		frames: Tensor of shape [T, C, H, W] containing episode frames
		encoder: PretrainedEncoder instance
		target_frames: Tensor of shape [N, C, H, W] containing goal frames
	...
	Returns:
		...
		target_embeddings: Tensor of shape [N, 768] — goal embeddings
		min_pixel_mse: Tensor of shape [T] — min pixel MSE to any goal at each timestep
		mean_pixel_mse: Tensor of shape [T] — mean pixel MSE to all goals at each timestep
	"""
	# Encode all trajectory frames
	embeddings = encoder(frames)  # [T, 768]

	# Encode all goal frames
	target_embeddings = encoder(target_frames)  # [N, 768]
	...
	if distance_metric == 'cosine':
		...
		target_norm = target_embeddings / (target_embeddings.norm(dim=-1, keepdim=True) + 1e-8)
		# Cosine similarity matrix [T, N]
		cosine_sim = embeddings_norm @ target_norm.T
		dist_matrix = 1 - cosine_sim
	elif distance_metric == 'l2':
		# [T, 1, 768] - [1, N, 768] -> [T, N, 768] -> [T, N]
	...
	# Chamfer distance
	# Forward: for each trajectory frame, nearest goal distance -> average
	forward_chamfer = dist_matrix.min(dim=1).values.mean()  # scalar
	# Backward: for each goal frame, nearest trajectory frame distance -> average
	backward_chamfer = dist_matrix.min(dim=0).values.mean()  # scalar
	chamfer_distance = (forward_chamfer + backward_chamfer).item()

	# Compute pixel MSE
	frames_float = frames.float()  # [T, C, H, W]
	target_float = target_frames.float()  # [N, C, H, W]
	# Pairwise pixel MSE: [T, N]
	# Compute efficiently by expanding: [T, 1, C, H, W] vs [1, N, C, H, W]
	pixel_mse_matrix = ((frames_float.unsqueeze(1) - target_float.unsqueeze(0)) ** 2).mean(dim=(2, 3, 4))

	min_pixel_mse = pixel_mse_matrix.min(dim=1).values  # [T]
	mean_pixel_mse = pixel_mse_matrix.mean(dim=1)  # [T]

	return (min_distances.cpu(), mean_distances.cpu(), chamfer_distance,
			embeddings.cpu(), target_embeddings.cpu(),
			min_pixel_mse.cpu(), mean_pixel_mse.cpu())


def plot_set_embedding_distances(min_dist_list, mean_dist_list, chamfer_list, task_names,
								 min_mse_list, mean_mse_list, done_indices_list, save_path, distance_metric='cosine', num_goals=0):
	"""
	Plot set-based embedding distances for multiple episodes.

	Args:
		min_dist_list: List of min distance tensors, each [T]
		mean_dist_list: List of mean distance tensors, each [T]
		chamfer_list: List of Chamfer distance scalars
		task_names: List of task names
		min_mse_list: List of min pixel MSE tensors, each [T]
		mean_mse_list: List of mean pixel MSE tensors, each [T]
		done_indices_list: List of done indices (when task was completed)
		save_path: Path to save the plot
		distance_metric: Type of distance metric used
		num_goals: Number of goal frames in the set
	"""
	...
	for idx, (min_dist, mean_dist, chamfer, min_mse, mean_mse, task_name, done_idx) in enumerate(
		zip(min_dist_list, mean_dist_list, chamfer_list, min_mse_list, mean_mse_list, task_names, done_indices_list)):

		timesteps = np.arange(len(min_dist))

		# Column 1: Min embedding distance
		ax1 = axes[idx, 0]
		...
		ax1.set_title(f'Task: {task_name} | Min Distance to Goal Set ({num_goals} goals)', fontsize=14, fontweight='bold')
		ax1.set_ylim(bottom=0)
		ax1.grid(True, alpha=0.3)
		ax1.legend(fontsize=10, loc='best')

		# Stats + Chamfer annotation
		...
		# Column 2: Mean embedding distance
		ax2 = axes[idx, 1]
		...
		ax2.set_title(f'Task: {task_name} | Mean Distance to Goal Set ({num_goals} goals)', fontsize=14, fontweight='bold')
		...
		# Column 3: Min pixel MSE
		ax3 = axes[idx, 2]
		ax3.plot(timesteps, min_mse.numpy(), linewidth=2, color='purple', label='Min pixel MSE')

		min_of_min_mse_idx = min_mse.argmin().item()
		ax3.axvline(x=min_of_min_mse_idx, color='green', linestyle='--', linewidth=2,
				   label=f'Closest (t={min_of_min_mse_idx})', alpha=0.7)
		ax3.scatter([min_of_min_mse_idx], [min_mse[min_of_min_mse_idx]], color='green', s=150, zorder=5, marker='o')

		ax3.axvline(x=done_idx, color='red', linestyle='--', linewidth=2,
				   label=f'Task Done (t={done_idx})', alpha=0.7)
		ax3.scatter([done_idx], [min_mse[done_idx]], color='red', s=150, zorder=5, marker='o')

		ax3.set_xlabel('Timestep', fontsize=12)
		ax3.set_ylabel('Pixel MSE', fontsize=12)
		ax3.set_title(f'Task: {task_name} | Min Pixel MSE to Goal Set', fontsize=14, fontweight='bold')
		ax3.set_ylim(bottom=0)
		ax3.grid(True, alpha=0.3)
		ax3.legend(fontsize=10, loc='best')

		stats_text_3 = (f'Min: {min_mse.min().item():.2f}\n'
						f'Mean: {min_mse.mean().item():.2f}\n'
						f'Std: {min_mse.std().item():.2f}')
		ax3.text(0.02, 0.98, stats_text_3, transform=ax3.transAxes, fontsize=10,
				verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

		# Column 4: Mean pixel MSE
		ax4 = axes[idx, 3]
		ax4.plot(timesteps, mean_mse.numpy(), linewidth=2, color='brown', label='Mean pixel MSE')

		min_mean_mse_idx = mean_mse.argmin().item()
		ax4.axvline(x=min_mean_mse_idx, color='green', linestyle='--', linewidth=2,
				   label=f'Closest (t={min_mean_mse_idx})', alpha=0.7)
		ax4.scatter([min_mean_mse_idx], [mean_mse[min_mean_mse_idx]], color='green', s=150, zorder=5, marker='o')

		ax4.axvline(x=done_idx, color='red', linestyle='--', linewidth=2,
				   label=f'Task Done (t={done_idx})', alpha=0.7)
		ax4.scatter([done_idx], [mean_mse[done_idx]], color='red', s=150, zorder=5, marker='o')

		ax4.set_xlabel('Timestep', fontsize=12)
		ax4.set_ylabel('Pixel MSE', fontsize=12)
		ax4.set_title(f'Task: {task_name} | Mean Pixel MSE to Goal Set', fontsize=14, fontweight='bold')
		ax4.set_ylim(bottom=0)
		ax4.grid(True, alpha=0.3)
		ax4.legend(fontsize=10, loc='best')

		stats_text_4 = (f'Min: {mean_mse.min().item():.2f}\n'
						f'Mean: {mean_mse.mean().item():.2f}\n'
						f'Std: {mean_mse.std().item():.2f}')
		ax4.text(0.02, 0.98, stats_text_4, transform=ax4.transAxes, fontsize=10,
				verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

	plt.tight_layout()
	plt.savefig(save_path, dpi=150, bbox_inches='tight')
	plt.close()
	print(f'Saved set-based distance plot to {save_path}')


def plot_embedding_trajectories_2d_set(embeddings_list, task_names, target_embeddings, save_path):
	"""
	Plot 2D trajectory of embeddings using PCA projection with goal set.

	Args:
		embeddings_list: List of embedding tensors, each of shape [T, 768]
		task_names: List of task names
		target_embeddings: Goal set embeddings of shape [N, 768]
		save_path: Path to save the plot
	"""
	...
	# Concatenate all embeddings + all target embeddings for PCA
	all_embeddings = torch.cat(embeddings_list + [target_embeddings], dim=0).numpy()
	...
	for embeddings in embeddings_list:
		...
		start_idx = end_idx
	target_embeddings_2d = all_embeddings_2d[start_idx:]  # Remaining are target embeddings
	...
	for idx, (embeddings_2d, task_name) in enumerate(zip(embeddings_2d_list, task_names)):
		...
		# Highlight all goal frames as a cluster
		ax.scatter(target_embeddings_2d[:, 0], target_embeddings_2d[:, 1],
				  color='red', s=200, marker='*', label=f'Goals ({len(target_embeddings_2d)})', zorder=5,
...
def plot_embedding_trajectories_3d_umap_set(embeddings_list, task_names, target_embeddings, save_path):
	"""
	Plot 3D trajectory of embeddings using UMAP projection with goal set.
	...
	# Concatenate all embeddings + all targets for UMAP
	all_embeddings = torch.cat(embeddings_list + [target_embeddings], dim=0).numpy()
	...
	for embeddings in embeddings_list:
		...
		start_idx = end_idx
	target_embeddings_3d = all_embeddings_3d[start_idx:]  # Remaining are targets
	...
	for idx, (embeddings_3d, task_name) in enumerate(zip(embeddings_3d_list, task_names)):
		...
		# Highlight all goal frames as a cluster (red stars)
		ax.scatter(target_embeddings_3d[:, 0], target_embeddings_3d[:, 1], target_embeddings_3d[:, 2],
				  color='red', s=400, marker='*', label=f'Goals ({len(target_embeddings_3d)})', zorder=10,
				  edgecolors='black', linewidths=1.5, alpha=0.8)
...
def generate_demos(cfg):
	"""Generates demonstrations with set-based embedding distance analysis using multiple goal frames."""
	...
	# Check for target frames directory
	assert hasattr(cfg, 'target_frames_dir'), \
		'Please specify path to target frames directory via +target_frames_dir=<path_to_directory>'
	target_frames_dir = cfg.target_frames_dir
	...
	# Load all goal frames from directory
	print(f'Loading goal frames from {target_frames_dir}...')
	target_frames = load_target_frames(target_frames_dir)  # [N, C, H, W]
	num_goals = target_frames.shape[0]
	...
	target_embeddings_stored = None  # Will be set on first computation

	# Generate demos
	print(f'Generating {cfg.num_demos} demonstrations with set-based embedding analysis ({num_goals} goals)...')
	demos_collected = 0
		...
		if done.any():
			...
			for i in range(cfg.num_envs):
				...
				if demos_collected >= cfg.num_demos:
					...
					# Compute set-based embedding distances
					print(f'Computing set-based distances for demo {demos_collected+1} ({num_goals} goals)...')
					(min_distances, mean_distances, chamfer_distance,
					 embeddings, target_embs,
					 min_pixel_mse, mean_pixel_mse) = compute_set_embedding_distances(
						ep_frames.to('cuda'), vision_encoder, target_frames.to('cuda'), distance_metric=distance_metric)
					...
					if target_embeddings_stored is None:
						target_embeddings_stored = target_embs

					# Print summary
					min_of_min_idx = min_distances.argmin().item()
					print(f'  Closest frame to any goal: t={min_of_min_idx} (min_dist={min_distances[min_of_min_idx]:.4f})')
	...
	# Plot set-based embedding distances
	print('\nGenerating set-based distance plots...')
	plot_save_path = f'{cfg.data_dir}/{cfg.task}_set_distances_{distance_metric}.png'
	plot_set_embedding_distances(min_dist_list, mean_dist_list, chamfer_list, accepted_task_names,
								min_mse_list, mean_mse_list, done_indices_list, plot_save_path, distance_metric=distance_metric, num_goals=num_goals)

	# Plot embedding trajectories (PCA 2D)
	print('Generating embedding trajectory plots (PCA 2D)...')
	trajectory_save_path = f'{cfg.data_dir}/{cfg.task}_embedding_trajectories_set_{distance_metric}.png'
	plot_embedding_trajectories_2d_set(embeddings_list, accepted_task_names, target_embeddings_stored, trajectory_save_path)

	# Plot embedding trajectories (UMAP 3D)
	print('Generating embedding trajectory plots (UMAP 3D)...')
	trajectory_save_path_umap = f'{cfg.data_dir}/{cfg.task}_embedding_trajectories_umap3d_set_{distance_metric}.png'
	plot_embedding_trajectories_3d_umap_set(embeddings_list, accepted_task_names, target_embeddings_stored, trajectory_save_path_umap)
	...
	print(f'Goal frames directory: {target_frames_dir} ({num_goals} goals)')
