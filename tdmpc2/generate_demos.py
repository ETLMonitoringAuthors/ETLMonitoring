...
def generate_demos(cfg):
	...
	# Save each demo separately
	# ncols = 1  # <- change this to control wrap
	# for d in range(frames.shape[0]):
	# 	demo = frames[d].float() / 255.0  # [T+1, C, H, W]
	# 	grid = make_grid(demo, nrow=ncols, padding=0)
	# 	save_image(grid, f'{cfg.data_dir}/{cfg.task}_demo_{d:03d}.png')

	# Save all demos in a single image (may be very wide)
